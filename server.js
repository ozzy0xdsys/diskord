...
app.get("/api/me", (req, res) => {
  ...
  return res.json({ user: safeUser(user), avatar: user.avatar || null });
});
...
app.post("/api/avatar", (req, res) => {
  ...
  const d = (dataUrl || "").toString();
  // Expect data:image/*;base64,...
  if (!/^data:image\/(png|jpeg|jpg|webp);base64,/.test(d)) {
    return res.status(400).json({ error: "Invalid image." });
  }
  if (d.length > 250000) return res.status(400).json({ error: "Image too large." });
...
// code -> { creatorId, sockets:Set(ws), memberCounts:Map(userId->count), leaveTimers:Map(userId->timeout) }
...
function ensureSession(code, creatorId) {
  if (!sessions.has(code)) {
    sessions.set(code, { creatorId, sockets: new Set(), memberCounts: new Map(), memberNames: new Map(), leaveTimers: new Map() });
  }
...
function listMembers(code) {
  ...
  if (!s) return [];
  const out = [];
  for (const [userId, username] of s.memberNames.entries()) {
...
function leaveSession(ws, code, reason = "leave") {
  ...
  if (next === 0) {
    if (reason === "disconnect") {
      // Grace period so refresh doesn't immediately show leave.
      clearLeaveTimer(s, ws.userId);
...
function removeSocketFromAll(ws, reason = "disconnect") {
  for (const code of Array.from(ws.sessionCodes)) {
    leaveSession(ws, code, reason);
  }
}
...
wss.on("connection", (ws, req) => {
  try {
    ...
    wsSend(ws, { type: "hello", user: safeUser(user), avatar: user.avatar || null });
    ...
    ws.on("message", (buf) => {
      ...
      if (msg.type === "create_session") {
        ...
        const s = ensureSession(code, ws.userId);
        joinSession(ws, code);
