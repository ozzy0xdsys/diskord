...
def app_page():
    ...
    try:
        ...
        if selected_id:
            ...
            messages = [{"id": m.id, "content": m.content, "created_at": m.created_at.isoformat(), "username": m.user.username, "attachment_name": m.attachment_name, "has_attachment": bool(m.attachment_path)} for m in msgs_db]
...
def groups_create():
    ...
    try:
        ...
        if not selected:
            return redirect("/app")
        convo = Conversation(type="group", name=random_group_name())
        db.add(convo); db.commit(); db.refresh(convo)
...
def api_messages(conversation_id: str):
    ...
    try:
        ...
        msgs_db = (db.query(Message)
                   ...
                   .limit(200).all())
        msgs_db = list(reversed(msgs_db))
        return jsonify([{"id": m.id, "content": m.content, "created_at": m.created_at.isoformat(), "username": m.user.username, "attachment_name": m.attachment_name, "has_attachment": bool(m.attachment_path)} for m in msgs_db])
    finally:
        db.close()
...
def sio_send(data):
    ...
    try:
        ...
        payload = {"id": msg.id, "conversationId": convo_id, "content": msg.content,
                   "createdAt": msg.created_at.isoformat(), "user": {"username": user.username}}
    finally:
        db.close()
    socketio.emit("message_new", payload, room=f"convo:{convo_id}")
