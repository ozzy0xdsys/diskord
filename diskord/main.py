...
def conversations_invite():
    ...
    try:
        ...
        raw = [x.strip() for x in (request.form.get("member_ids") or "").split(",") if x.strip()]
        if not convo_id or not raw:
            return redirect("/app")
        convo = db.query(Conversation).filter(Conversation.id == convo_id).first()
        if not convo:
            return redirect("/app")
        # must be a member to invite
        if not is_member(db, convo_id, user.id):
            abort(403)
        # announcement: only owner can invite
        if convo.type == "announcement" and convo.owner_id != user.id:
            abort(403)
        ...
        existing = {m.user_id for m in db.query(ConversationMember).filter(ConversationMember.conversation_id == convo_id).all()}
        for uid in set(selected):
            if uid in existing:
                continue
            db.add(ConversationMember(conversation_id=convo_id, user_id=uid, role="member"))
        db.commit()
        return redirect("/app")
    finally:
        db.close()
...
def profile_page():
    ...
    try:
        user, sd = current_user_or_401(db)
        return render_template("profile.html", username=user.username, csrf=sd["csrf"], avatar_url=(f"/avatars/{user.id}" if user.avatar_path else None))
    finally:
        db.close()
...
def sio_join(data):
    ...
    try:
        ...
        if not convo:
            return
        # members can join announcements to receive updates (read-only enforced on send)
    finally:
        db.close()
